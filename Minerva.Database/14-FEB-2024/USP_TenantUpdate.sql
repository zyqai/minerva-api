USE `minerva`;
DROP procedure IF EXISTS `USP_TenantUpdate`;

USE `minerva`;
DROP procedure IF EXISTS `minerva`.`USP_TenantUpdate`;
;
/*
Author: Santhosh
Created Date: 2024-02-01
Purpose: This procedure update tenants information in the '_tenants' table.

Modified Date: 2024-02-13
Modified By : santhosh
Purpose : added Created by and CreateDateTime 
*/
DELIMITER $$
USE `minerva`$$
CREATE DEFINER=`minerva_admin_dev`@`%` PROCEDURE `USP_TenantUpdate`(
    IN p_tenantId INT,
